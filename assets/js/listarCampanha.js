...
$(document).ready(function () {
	$.get("./api/empresa/" + JSON.parse(document.querySelector("head").getAttribute("data-session"))["ID_Empresa"] + "/campanha", function (data) {
		...
		campanhas.forEach(campanha => {
			$.get("./api/empresa/" + JSON.parse(document.querySelector("head").getAttribute("data-session"))["ID_Empresa"] + "/campanha/" + campanha["ID_Campanha"] + "/instanciascampanha", function (data) {
		...
		document.querySelectorAll("tr:not(#head)").forEach((row) => {
			row.addEventListener("click", function () {
				...
				document.querySelector("#modal .modal-footer a").href = "./infoCampanha/" + this.getAttribute("data-url");
