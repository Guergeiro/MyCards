		...
		data.forEach(dataElement => {
			...
			if (dataElement.Dono == 0) {
				...
				button.addEventListener("click", function () {
					$.ajax({
						url: encodeURI("./api/empresa/" + JSON.parse(document.querySelector("head").getAttribute("data-session"))["ID_Empresa"] + "/colaborador/" + dataElement.Nome),
						type: "DELETE",
...
document.querySelector(".modal-footer button.btn-primary").addEventListener("click", function () {
	...
	if (okay) {
		$.post("./api/empresa/" + JSON.parse(document.querySelector("head").getAttribute("data-session"))["ID_Empresa"] + "/colaborador", {
