...
document.querySelectorAll(".teste").forEach(div => {
	div.addEventListener("click", function () {
		if (div.classList.contains("text-muted")) {
			...
			addDataBar(chartEstatisticasGerais, meses, div.lastElementChild.innerHTML);
		} else {
			div.classList.add("text-muted");
			div.classList.remove("text-primary");
			removeData(chartEstatisticasGerais, meses);
		}
	});
});
...
$(document).ready(function () {
	$.get("./api/empresa/" + JSON.parse(document.querySelector("head").getAttribute("data-session"))["ID_Empresa"] + "/cartao", function (data) {
		...
		data.forEach((cartao) => {
			cartoes.push(cartao);
			$.get("./api/cliente/" + cartao["ID_Cliente"], function (cliente) {
	...
	$.get("./api/empresa/" + JSON.parse(document.querySelector("head").getAttribute("data-session"))["ID_Empresa"] + "/campanha", function (data) {
		...
		data.forEach((campanha) => {
			...
			$.get("./api/empresa/" + JSON.parse(document.querySelector("head").getAttribute("data-session"))["ID_Empresa"] + "/campanha/" + campanha["ID_Campanha"] + "/instanciacampanha", function (instancias) {
