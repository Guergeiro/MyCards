          ...
          function(instancias) {
            ...
            instancias.forEach(element => {
              if (!contains(element)) {
                ...
                let oldinnerHtml = document.querySelector(".card-body .row")
                  .innerHTML;
                ...
                document.querySelectorAll(".toast-body").forEach(toast => {
                  toast.addEventListener("click", function() {
                    ...
                    document.querySelector(
                      "#codigocampanha"
                    ).value = toast.getAttribute("data-idcampanha");
                    document.querySelector("#codigocampanha").focus();
                    document.querySelector("#valor").focus();
                  });
                });
...
function contains(element) {
  ...
  if (notificacoes.length > 0) {
    notificacoes.forEach(objElement => {
      if (
        objElement["ID_Cartao"] == element["ID_Cartao"] &&
        objElement["ID_Campanha"] == element["ID_Campanha"]
      ) {
        estado = true;
      }
    });
